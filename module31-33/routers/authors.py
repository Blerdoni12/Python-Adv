...
def create_author(author: AuthorCreate,api_key: str = Depends(get_api_key),):
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail=f"The author '{author.name}' already exists.",)
