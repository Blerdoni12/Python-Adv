...
@app.post(path="/items/", response_model=Item)  new *
def create_new_item(item: Item, api_key: str = Depends(get_api_key)):
    return create_item(item)

@app.get(path="/items/", response_model=List[Item])  new *
def read_items(api_key: str = Depends(get_api_key)):
    return get_items()

@app.get(path="/items/{item_id}", response_model=Item)  new *
...
@app.put(path="/items/{item_id}", response_model=Item)  new *
