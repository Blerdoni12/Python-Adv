...
st.subheader("Number if Fiction vs Non-Fiction Books over the Years")
...
st.subheader("Top 15 Authors by Counts of Books Published (2000–2022)")

top_authors = filtered_books_df['Author'].value_counts().head(15).reset_index()
top_authors.columns = ['Author', 'Count']

fig = px.bar(top_authors,x='Count',y='Author',orientation='h',title="Top 15 Authors by Counts of Books Published",labels={'Count': 'Counts of Books Published', 'Author': 'Author'},color='Count',color_continuous_scale=px.colors.sequential.Plasma)
