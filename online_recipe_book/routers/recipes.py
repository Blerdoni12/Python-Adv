...
def get_recipes(cuisine: str = None, difficulty: str = None):
    ...
    return [
        Recipe(
            id=row[0],name=row[1],description=row[2],ingredients=row[3], instructions=row[4],cuisine=row[5],difficulty=row[6],
            category_id=row[7]
        )
        for row in recipes
    ]
...
def create_recipe(recipe: RecipeCreate):
    ...
    cursor.execute(
        ...
        (
            recipe.name,recipe.description,recipe.ingredients,recipe.instructions, recipe.cuisine,recipe.difficulty,
            recipe.category_id,
        ),
    )
    ...
    return Recipe(
        id=recipe_id,name=recipe.name,description=recipe.description,ingredients=recipe.ingredients,
